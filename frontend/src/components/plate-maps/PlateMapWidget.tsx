...
// ─── Export button with theme flyout ─────────────────────────────────────────
...
function ExportButton({
  ...
  return (
    ...
    >
      <button
        type="button"
        onClick={() => onExport(currentTheme)}
        className="rounded bg-blue-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-blue-700"
      >
        {status}
      </button>
      {showFlyout && status === label && (
        <button
          type="button"
          onClick={() => {
          ...
          className="absolute left-0 top-full mt-1 z-10 whitespace-nowrap rounded border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-1.5 text-xs text-gray-700 dark:text-gray-300 shadow-lg hover:bg-gray-50 dark:hover:bg-gray-700"
        >
...
export interface PlateMapWidgetProps {
  plateId: string
  maxWidth?: number
  readOnly?: boolean
...
// ─── Inner widget (needs UndoProvider in scope) ───────────────────────────────

function PlateMapWidgetInner({ plateId, maxWidth, readOnly = false, onDirtyChange }: PlateMapWidgetProps) {
  const { data: pmData, isLoading } = usePlateMap(plateId)
  ...
  // Prevents well click from double-firing after a drag
  ...
  const doSave = useCallback(
    ...
    [plateId, readOnly, updateMutation]
  )
  ...
  useEffect(() => {
    return () => {
      ...
      if (userEdited.current) {
        ...
        fetch(`/api/v1/plate-maps/${plateId}`, {
          method: 'PUT',
  ...
  // ── Auto-clear selection on click outside ──
  useEffect(() => {
    function handleGlobalMouseDown(e: MouseEvent) {
      if (state.selectedWells.length === 0) return
      const target = e.target as HTMLElement
      const wellEl = target.closest('[data-well-id]') as HTMLElement | null
      if (wellEl) {
        const wellId = wellEl.dataset['wellId']
        if (wellId && state.selectedWells.includes(wellId)) return
      }
      if (target.closest('[data-controls-panel]')) return
  ...
  const handleWellClick = useCallback(
    ...
    [state.selectedWells, rowLabels, colLabels] // eslint-disable-line react-hooks/exhaustive-deps
  )
  ...
  const handleRowClick = useCallback(
    ...
    [rowLabels, colLabels] // eslint-disable-line react-hooks/exhaustive-deps
  )
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const target = e.target as HTMLElement
    // Header buttons (row/col selectors) don't start drag
    ...
    // If mousedown was on a well, immediately include it in selection
    const wellEl = target.closest('[data-well-id]') as HTMLElement | null
    if (wellEl) {
      const wellId = wellEl.dataset['wellId']
      if (wellId && !e.shiftKey) {
        const isCtrl = e.ctrlKey || e.metaKey
        if (!isCtrl) {
          dispatch({ type: 'SET_SELECTION', wells: [wellId] })
        }
  ...
  const handleMouseUp = useCallback(
    ...
    [dragRect, previewWells] // eslint-disable-line react-hooks/exhaustive-deps
  )
  ...
  const handleApplyColor = useCallback(
    ...
    [state.wellData, pushUndo] // eslint-disable-line react-hooks/exhaustive-deps
  )
  ...
  }, [state.wellData, state.legend, pushUndo]) // eslint-disable-line react-hooks/exhaustive-deps

  // ── PNG export ──
  const exportPNG = useCallback(async (mode: 'copy' | 'download', targetTheme: 'light' | 'dark') => {
    ...
    try {
      ...
      const needsToggle = (targetTheme === 'dark') !== wasDark
      ...
      try {
        canvas = await html2canvas(el, { backgroundColor: null, scale: 3 })
      } catch {
        canvas = await html2canvas(el, { backgroundColor: null, scale: 1 })
      }
      ...
      setTimeout(() => setExportStatus((prev) => ({
        ...prev,
        [key]: mode === 'copy' ? 'Copy PNG' : 'Download PNG',
      })), 2000)
    }
  }, [state.name])
      ...
      >
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <ExportButton
              label="Copy PNG"
              status={exportStatus.copy}
              currentTheme={theme}
              onExport={(t) => exportPNG('copy', t)}
            />
            <ExportButton
              label="Download PNG"
              status={exportStatus.download}
              currentTheme={theme}
              onExport={(t) => exportPNG('download', t)}
            />
          </div>

          <div
            ref={presentationRef}
            className="flex gap-8 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-6"
          >
            <div className="flex-1 min-w-0">
              <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100 mb-1">{state.name}</h2>
              {state.description && (
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">{state.description}</p>
              )}
              <PlateMapGrid
                plateType={state.plateType}
                wellData={state.wellData}
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4">
        ...
        <div className="flex gap-4">
          {/* Controls sidebar */}
          <div className="w-56 shrink-0" data-controls-panel="">
            <PlateMapControls
              plateType={state.plateType}
          ...
          <div className="w-56 shrink-0">
            ...
            <PlateMapLegendPanel
              ...
              onWellDataChange={(wellData) => {
                markEdited()
                dispatch({ type: 'SET_WELL_DATA', wellData })
              }}
            />
          </div>
        </div>
...
// ─── Public widget (wraps inner with UndoProvider) ────────────────────────────

export default function PlateMapWidget(props: PlateMapWidgetProps) {
  return (
    <UndoProvider>
      <PlateMapWidgetInner {...props} />
    </UndoProvider>
  )
}
